  ...
  stages {
    ...
    stage('backend_docker_container') {
      steps {
        ...
        // sh 'docker run --name ${BACKEND_CONTAINER} -p 8080:8080 ${BACKEND_IMAGE} java -Dspring.profiles.active=production -jar app.jar'
        sh 'docker run --name ${BACKEND_CONTAINER} -p 8080:8080 ${BACKEND_IMAGE} java ${PF_PROFILE}${PROFILE} -jar app.jar'
      }
    }
  }
}
